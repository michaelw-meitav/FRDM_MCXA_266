...
#define DEFAULT_MANAGEMENT_GETDEVFUNC()                                             \
  ...
  case LCD_DEVFUNC_GET_BUFFERPTR:                                                   \
    return (void (*)(void))_GetBufferPtr;                                           \
...
*       _GetBufferPtr
*/
static void * _GetBufferPtr(GUI_DEVICE * pDevice, int BufferIndex) {
  DRIVER_CONTEXT * pContext;
  void           * pBuffer;
  I32              BufferSize;
  int              BitsPerPixel;
  ...
  if (pDevice->u.pContext) {
    ...
    BufferSize   = (((U32)pContext->vxSize * pContext->ySize * BitsPerPixel) >> 3);
    if (pContext->aBufferPTR) {
      pBuffer = pContext->aBufferPTR[BufferIndex];
    } else {
      pBuffer = (U8 *)pContext->BaseAddr + (BufferSize * BufferIndex);
    }
  }
  return pBuffer;
}
